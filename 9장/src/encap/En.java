...
class ColdPatient //감기환자
{
	...
	CONTAC600 buyCONTAC600(Pharmacist pm, int money, int cnt_con){  //약사에게 돈을 주고 약을 산다.
	...
	Tilenol buyTylenol(Pharmacist pm, int money, int cnt_t){  //약사에게 돈을 주고 약을 산다.
		Tilenol t = pm.saleTilenol(money, cnt_t);  //약사는 돈을 받고 약을 판다.
		if(t != null) {
			lastmoney -= money;      
			this.cnt_t += cnt_t;
		}
	...
	void takeTylenol(Tilenol cap){ //타이레놀 먹기
		cap.take(); 
	} 
...
public class En
{
	...
	{   
		...
		Pharmacist pm = new Pharmacist("예담", 0, 1, 2);  //돈은 없고 콘택600은 100개,타이레놀 200개의 약을 가진 약사 객체 생성하여 그 주소를 pm에 저장
		   ...
		   /*System.out.println("\n손님, 콘택600은 1개에 " + Pharmacist5.CONTAC600_PRICE 
		    ...
		    switch(s1){		    
			  ...
			  case "타이레놀" :
				System.out.print(s1 + " 몇 개 드릴까요?(예:3) :" );
				cnt = Sc.s.nextInt();  //약의 개수를 입력받기, static이므로 클래스명Sc.참조변수s
				Tilenol t=p.buyTylenol(pm, cnt*Pharmacist.Tilenol_PRICE, cnt);   //주체:환자는 약사에게 약값을 주고 약개수만큼 약 사기
